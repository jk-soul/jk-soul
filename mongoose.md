...
Middleware
-----------
Middleware的功能是通过控制流执行期间文档init、验证、保存和删除方法。执行中间件在文档级别,而不是模型的水平。有两种类型的中间件,前置和后置。让我们从Pre开始。
Pre
----
有两种类型的pre中间件、串行和并行
Serial
-------
串行中间件是一个接一个地执行,当每个中间件调用next。
```javascript
var schema = new Schema(..);
schema.pre('save', function (next) {
  // do stuff
...
Use Cases
----------
中间件是很有用的雾化模型逻辑,避免嵌套的异步代码块。以下是一些想法:
-  复杂的验证
-  删除相关文件
-  (删除用户删除他所有的言论)
-  异步违约
...
Error handling
-----------------
如果任何中间件调用下一个或完成一个错误实例,流程中断,错误被传递给回调函数。
```javascript
schema.pre('save', function (next) {
  var err = new Error('something went wrong');
  next(err);
